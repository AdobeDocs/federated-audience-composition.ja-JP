...
>abstract="「**[!UICONTROL 出力母集団の重複をを有効にする]**」オプションにより、複数のサブセットに属している母集団を管理できます。このチェックボックスをオンにしていない場合、分割アクティビティは、受信者が複数のサブセットの条件を満たしている場合でも、受信者が複数の出力トランジションに表示できないようにします。受信者は、一致した基準の 1 番目のタブのターゲットになります。チェックボックスをオンにすると、受信者がフィルター条件を満たす場合、複数のサブセットで受信者を検索できます。 "
