...
**[!UICONTROL WIF]**&#x200B;を選択した場合、**not**&#x200B;はログイン情報を提供する必要があります。 ただし、**は** クライアントライブラリ設定を&#x200B;**[!UICONTROL キーファイルパス]**&#x200B;として追加する必要があります。 クライアントライブラリの設定について詳しくは、[Google BigQuery （Workload Identity Federation）の設定セクション ](#wif-configuration)を参照してください。
...
### Google BigQuery （Workload Identity Federation）設定 {#wif-configuration}

Google Cloud Platformの設定を行う前に、次の値が必要です。

- AWS アカウント ID
   - この値を取得するには、Adobe担当者にお問い合わせください。
- AWS IAM ロール名
   - AWS IAM ロール名は、次の形式に従います：`arn:aws:iam::<ADOBE_AWS_ACCOUNT_ID>:role/fac-<CUSTOMER_IMS_ORG_ID>`

Google Cloud Consoleで、**IAMと管理者セクション**&#x200B;に&#x200B;**Workload ID プール**&#x200B;を作成します。 これにより、外部IDを整理および管理できます。

「**プロバイダーを追加**」を選択して、ID プロバイダーを作成します。 これにより、プロバイダーに関する関連メタデータを提供することで、Google CloudのID プロバイダーとWorker ID プールとの間で一方向の信頼が設定されます。

![ プロバイダーを追加ボタンがGoogle Cloudでハイライト表示されます。](/help/connections/assets/home/select-add-provider.png)

プロバイダーを作成する場合は、次の情報を提供する必要があります。

| フィールド | 説明 |
| ----- | ----------- |
| 名前 | ワークロード ID プール プロバイダーの名前。 |
| ID | プロバイダーのIDは自動的に生成されます。 |
| AWS アカウント ID | 以前に指定したAWS アカウント ID。 |
| 有効なプロバイダー | プロバイダーを決定するブール値が有効または無効です。 |
| 属性マッピング | 役割に一致するマッピング。 この情報は既に存在します。 |

プロバイダーを作成した後、ワークロード ID プール IDがサービスアカウントを偽装できるように、IAM ポリシーを作成する必要があります。 「**アクセス権を付与**」を選択して、サービスアカウントへのアクセス権を付与ダイアログを開きます。
...
![ アクセス権の付与ダイアログが表示されます。](/help/connections/assets/home/aws_role.png)

サービスアカウントへのアクセス権を付与したら、クライアントライブラリ設定をダウンロードします。

![ ライブラリ設定をダウンロードする場所が表示されます。](/help/connections/assets/home/download-config.png)

クライアントライブラリ設定をダウンロードした後、Federated Audience ConfigurationでWIF接続を設定できるようになりました。
