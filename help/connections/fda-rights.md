...
| **テーブルへのデータの読み込み** | ターゲットテーブル権限での `CREATE STAGE ON SCHEMA`、`Create file FORMATGRANT CREATE FILE FORMAT ON SCHEMA <SCHEMA> to ROLE <ROLE>` `SELECT`、`INSERT` | `SELECT` および `INSERT` 権限 | サービスアカウントに割り当てられる役割には、`bigquery.jobs.create`、`bigquery.tables.getData`、`bigquery.tables.updateData` が含まれている必要があります | `SELECT` および `MODIFY` 権限 |
| **クライアントデータへのアクセス** | `SELECT on (FUTURE) TABLE(S)` または `VIEW(S)` 権限 | `SELECT` 権限 | サービスアカウントに割り当てられる役割には、テーブル用の `bigquery.jobs.create`、`bigquery.readsessions.create` および `bigquery.tables.getData` や、`bigquery.dataViewer` 役割が含まれている必要があります | `SELECT` 権限 |
...
| **実行のモニタリング** | ワークスペースコントリビューターまたは上記の権限（`queryinsights.exec_requests_history`） | `CONTROL` 権限 | `EXPLAIN` 文の使用に権限は不要 | `EXPLAIN` の使用に追加の権限は不要 |
